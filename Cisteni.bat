...
net session >nul 2>&1
if %errorLevel% == 0 (
    echo [OK] THIS SCRIPT WAS RUN AS ADMIN.
) else (
...
echo ==================================================
echo [INFO] Waiting 20s for the choice.... (Default: 2)...
echo.
...
if "%REZIM%"=="2" (
    ...
    reg delete "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v DisallowCpl /f >nul 2>&1
